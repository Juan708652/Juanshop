...
# ========== API TOP UP BOY (SESUAI DOKUMENTASI) ==========
TOPUPBOY_API_KEY = "YTiobTi9-UPQegDAS-msQPjh4i-Cszu80YN"
TOPUPBOY_BASE_URL = "https://topupboy.com/api/v1"

# ========== MAPPING PRODUK (product_id dari API Top Up Boy) ==========
# Kamu harus dapatkan product_id asli dari API /product Top Up Boy
# Untuk sementara, ini contoh placeholder - GANTI DENGAN ASLI!
PRODUCT_ID_MAP = {
    'ff_5': 0,      # GANTI dengan product_id 5 Diamond
    'ff_10': 0,     # GANTI dengan product_id 10 Diamond
    'ff_20': 0,     # GANTI dengan product_id 20 Diamond
    'ff_50': 0,     # GANTI dengan product_id 50 Diamond
    'ff_100': 0,    # GANTI dengan product_id 100 Diamond
    'ff_200': 0,    # GANTI dengan product_id 200 Diamond
    'ff_140': 0,    # GANTI dengan product_id 140 Diamond
    'ff_130': 0,    # GANTI dengan product_id 130 Diamond
    'ff_145': 0,    # GANTI dengan product_id 145 Diamond
    'ff_125': 0,    # GANTI dengan product_id 125 Diamond
    'ff_90': 0,     # GANTI dengan product_id 90 Diamond
    'ff_300': 0,    # GANTI dengan product_id 300 Diamond
}

# ========== PRODUK ==========
PRODUCTS = {
    'ff_5': {'name': '5 Diamonds', 'game': 'FF', 'price': 1400},
...
def topup_via_api(user_id, product_id, ref_id):
    """Kirim topup ke Top Up Boy sesuai dokumentasi"""
    try:
        headers = {
        ...
        url = f"{TOPUPBOY_BASE_URL}/transaction"
        ...
        if response.status_code == 200:
            ...
            if result.get('status') == True:
                return {'success': True, 'order_id': result.get('data', {}).get('order_id')}
            else:
...
📞 Nomor DANA: {DANA_NUMBER}
        ...
        app.orders.append({
            ...
            'product_id': PRODUCT_ID_MAP.get(data.get('product_code'), 0),
            'price': harga_bayar,
...
def process_topup():
    ...
    if not product_id or product_id == 0:
        return jsonify({'success': False, 'message': 'Product ID tidak valid. Hubungi admin untuk mapping product_id'}), 400

    result = topup_via_api(order['customer_id'], product_id, order['order_id'])
    
    if result['success']:
        order['status'] = 'completed'
        order['ref_id'] = result.get('order_id')
        kirim_ke_telegram(f"✅ TOPUP BERHASIL!\n🆔 Order ID: {order_id}\n👤 User ID: {order['customer_id']}\n💎 Produk: {order['product_name']}")
        return jsonify({'success': True, 'message': f'Topup berhasil! Diamond sudah dikirim ke {order["customer_id"]}'})
    else:
...
# ========== ENDPOINT UNTUK MENDAPATKAN PRODUCT_ID (SEMENTARA) ==========
@app.route('/api/fetch-products')
@login_required
def fetch_products():
    """Endpoint sementara untuk mendapatkan daftar produk dari Top Up Boy"""
    try:
        headers = {
            "Authorization": f"Bearer {TOPUPBOY_API_KEY}",
            "Content-Type": "application/json"
        ...
        # Langkah 1: Dapatkan games_id untuk Free Fire
        games_response = requests.post(f"{TOPUPBOY_BASE_URL}/games", headers=headers, json={}, timeout=30)
        games_data = games_response.json()
        
        if not games_data.get('status'):
            return jsonify({'error': 'Gagal mendapatkan games', 'detail': games_data})
        
        # Cari games_id untuk Free Fire
        games_id = None
        for game in games_data.get('data', []):
            if 'free fire' in game.get('games', '').lower():
                games_id = game.get('id')
                break
        
        if not games_id:
            return jsonify({'error': 'Game Free Fire tidak ditemukan'})
        
        # Langkah 2: Dapatkan produk berdasarkan games_id
        product_response = requests.post(f"{TOPUPBOY_BASE_URL}/product", headers=headers, json={"games_id": games_id}, timeout=30)
        products_data = product_response.json()
        
        return jsonify({
            'games_id': games_id,
            'products': products_data.get('data', {}).get('product', [])
        })
    except Exception as e:
